// ==================== VARIABLES GLOBALES ====================
      ...
      try {
        ...
        historyList.innerHTML = history.map((item, index) => {
          const statusBadge = item.approved ? 
            '<span style="background: rgba(255, 140, 0, 0.2); color: #FF8C00; padding: 0.25rem 0.75rem; border-radius: 12px; font-size: 0.85rem;">✓ Aprobada</span>' : 
    ...
    async function generatePDF() {
      ...
      y += 10;
      doc.setFontSize(16);
      doc.text('COTIZACIÓN SISTEMA CLIENTES', pageWidth / 2, y, { align: 'center' });
    ...
    document.getElementById('btnSavePrice').addEventListener('click', async () => {
      ...
      const priceClientesInput = document.getElementById('priceClientes').value.trim();
      ...
      if (!name || !category || !priceClientesInput) {
        alert('⚠️ Completá los campos obligatorios:\n- Nombre del producto\n- Categoría\n- Precio Clientes');
        return;
      }
      
      const priceClientes = parseFloat(priceClientesInput);
      ...
      if (isNaN(priceClientes) || priceClientes <= 0) {
        alert('⚠️ El precio Clientes debe ser un número válido mayor a 0');
        return;
      }
      ...
      if (success) {
        ...
        document.getElementById('priceClientes').value = '';
      ...
      container.innerHTML = precios.map(p => `
        <div class="product-item">
          <div class="product-info">
            <h4>${p.name}</h4>
            <div class="product-details">
              Clientes: $${p.priceClientes.toFixed(2)} | Público: $${p.pricePublico.toFixed(2)} | ${p.unit}
    ...
    console.log('✅ Sistema Clientes v3.6 CORREGIDO iniciado correctamente');